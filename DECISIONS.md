...
# Phase 2 Refinements (post-live-testing)

## LinkedIn insights cache

**Choice:** The "What PowerPost noticed" line is generated by a small LLM call grounded in the user's top + bottom-engagement posts plus voice profile, then cached on `linkedin_accounts.insight_text` with a `insight_generated_at` timestamp. TTL: 24 hours, regenerated on next dashboard load if stale or null.

**Why:** The dashboard is hit often. Generating a fresh insight on every load would be wasteful both for cost and latency. The 24h window lines up with the once-daily sync cadence we expect in production. Insight gets a forced refresh whenever a new sync runs (next migration if needed: bust on `last_synced_at` change).

**Sparse-history fallback:** Fewer than 3 posts → we return a plain "Not enough post history yet..." copy without an LLM call.

## Workshop post_goal

**Choice:** Added `workshop_sessions.post_goal` (TEXT, nullable). The eight goal values from the brief are validated server-side via Zod. The LLM system prompt now ends with a goal-specific clause appended via `postGoalContext()` in `routes/workshop.ts`. "Just sound like me" specifically tells the model not to optimize for engagement.

**Why:** The brief flagged that workshop drafts felt under-shaped. Pinning a goal at session start changes both the first draft and every subsequent rationale meaningfully — and gives the user a concrete escape hatch ("Just sound like me") when they want PowerPost to back off engagement-tuning.

**Required field:** The web start screen disables Start until a goal is picked. The API treats it as optional (the prompt simply doesn't append the goal clause if missing) so legacy sessions still load.

## Improve handoff from Workshop

**Choice:** "Improve this draft" on a Workshop draft writes `{ draft, workshop_id }` to `sessionStorage` under `pp_improve_handoff` and navigates to `/improve`. The Improve page reads the handoff on mount, prefills the textarea, and surfaces a "From Workshop draft" chip with a Back link. On Analyze, the API call includes `source_workshop_id` which lands in `improvement_suggestions.source_workshop_id`.

**Why:** URL params don't fit large drafts cleanly. SessionStorage gives us a one-shot handoff that survives a navigation but isn't permanent. The traceability column lets future analytics ask "what fraction of Improve sessions come from Workshop drafts and how do their score deltas compare to fresh pastes."

## Combined Improve target picker

**Choice:** Replaced the seven-button KPI picker with eleven options grouped into "what kind of optimization" (Just sound like me, Voice alignment, Performance, Balanced) and per-KPI options (Impressions, Comments, etc). Each option maps to a `(target, kpi?)` tuple sent to the API.

**Why:** The brief moved Improve from "pick a KPI" to "pick what you want this draft to do." The new shape lets the LLM pick a path appropriately AND the optimize buttons after analysis stay aligned with the user's stated preference (the `target` is now stored on `improvement_suggestions.optimization_target` for future reference and analytics).

## ThinkingState component

**Choice:** A single `<ThinkingState>` component with three render variants (`card`, `inline`, `bubble`), a default rotating message list, and reduced-motion fallback. Drop in anywhere we're awaiting an LLM response.

**Why:** Per the brief, "feels frozen" was a real user complaint. A premium animated indicator with rotating PowerSpeak-style status messages addresses both perceived speed and brand voice. Using one component everywhere keeps the feel consistent.

**Reduced motion:** When `prefers-reduced-motion: reduce` is set, the rotation pauses and the user sees the first message only.

## Logo tone prop

**Choice:** Replaced the implicit `square-light` variant with an explicit `tone="dark" | "light"` prop on the `Logo` component. Dark = the asset designed for placement on a light surface (current default). Light = the asset designed for placement on a dark surface.
